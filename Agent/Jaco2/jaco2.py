#!/home/stayalive/anaconda3/envs/AI_and_Science/bin/python
# coding=utf-8
"""
@author Bruce Hou
@brief Jaco2Agent
...
class Jaco2:
    ...
    def __init__(
        ...
        self.maxForce = 200  # << 机械臂机体最大转动力
        self.maxVelocity = 0.1  # << 机械臂转动速度
        self.fingerVelocity = 0.1  # << 机械臂抓取器转动速度
        self.finger1Force = 2.5  # << 机械臂手指1的转动力
        self.finger2Force = 2.5  # << 机械臂手指2的转动力
        self.finger3Force = 2  # << 机械臂手指3的转动力
        self.fingerTipForce = 5  # << 指尖力
        self.fingerPos = 0  # << 手指当前位置
        self.fingerTipPos = 0  # << 指尖当前位置
    ...
    def getEndeffectorState(self):
        """
        @brief 获取末端执行器高度
        """
        state = p.getLinkState(self.jacoUid, self.jacoEndEffectorIndex)
        # 三维坐标
        pos = state[4]
        # 四元数
        qua = state[5]
    ...
    def applyAction(self, action):
        ...
        if self._useAngleSpace:
            ...
            if self._jacoModel[:5] == "j2n6s":
                # 先移动到目标位置
                dxyz = np.array(action[:3]) / self._actionApplyTime
                # 夹角控制
                # 当时用开关信号的时侯，需要进行位姿控制，但是机械臂夹爪始终竖直向下
                # 如果使用开关信号，倒数第二个量便是夹爪的角度增量值
                # 倒数第一个量是夹爪的开关信号
                self._gripperAngle = self._gripperAngle + action[3]
                ...
                for _ in range(self._actionApplyTime):
                    ...
                    # 限制x轴
                    if self._jacoEndEffectorPosition[0] > 0.1:
                        self._jacoEndEffectorPosition[0] = 0.1
                    if self._jacoEndEffectorPosition[0] < -0.1:
                        self._jacoEndEffectorPosition[0] = -0.1
                    # 限制y轴
                    if self._jacoEndEffectorPosition[1] > 0.2:
                        self._jacoEndEffectorPosition[1] = 0.2
                    if self._jacoEndEffectorPosition[1] < -0.2:
                        self._jacoEndEffectorPosition[1] = -0.2
                    # 限制z轴
                    if self._jacoEndEffectorPosition[2] < 0.0:
                        self._jacoEndEffectorPosition[2] = 0.0
                ...
                if self._jacoModel[5:8] == "200":
                    # 最后一位便是开关信号
                    d_fingerPos = (
                        (1.4 - 0.3) * (action[len(action) - 1] - self.fingerPos) / 100
                    )
                    d_fingerTipPos = (
                        (2 - 0.5) * (float(action[len(action) - 1]) - self.fingerTipPos)
                    ) / 60

                    if self.fingerPos == 1:
                        finger_start_pos = 1.4
                        fingerTip_start_pos = 2
                    else:
                        finger_start_pos = 0.3
                        fingerTip_start_pos = 0.4
                    ...
                    for i in range(100):
                        for j in range(2):
                            p.setJointMotorControl2(
                                ...
                                8 + j * 2,
                                p.POSITION_CONTROL,
                                targetPosition=d_fingerPos * i + finger_start_pos,
                                force=self.finger1Force,
                            )
                            if action[len(action) - 1] == 1:
                                # if i >= 40:
                                p.setJointMotorControl2(
                                    ...
                                    9 + j * 2,
                                    p.POSITION_CONTROL,
                                    # targetPosition=fingerTip_start_pos
                                    # + d_fingerTipPos * i,
                                    targetPosition=0.7,
                                    force=self.finger2Force,
                                )
                        p.stepSimulation()
                    self.fingerPos = action[len(action) - 1]
                    self.fingerTipPos = action[len(action) - 1]

                elif self._jacoModel[5:8] == "300":
                    # 最后一位便是开关信号
                    d_fingerPos = (
                        (2 - 0.5)
                        * (float(action[len(action) - 1]) - self.fingerPos)
                        / 100
                    )
                    d_fingerTipPos = (
                        (2 - 0.5) * (float(action[len(action) - 1]) - self.fingerTipPos)
                    ...
                    if self.fingerPos == 1:
                        ...
                        finger_start_pos = 0.5
                        fingerTip_start_pos = 0.5
                    for i in range(100):
                        for j in range(3):
                            p.setJointMotorControl2(
                                self.jacoUid,
                                8 + j * 2,
                                p.POSITION_CONTROL,
                                targetPosition=finger_start_pos + d_fingerPos * i,
                                force=self.finger1Force,
                            )
                            if action[len(action) - 1] == 1:
                                if i >= 40:
