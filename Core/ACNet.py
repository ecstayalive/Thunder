...
from .ImgStack import ImgStack
from .StateStack import StateStack
from .InitWeight import initWeights
...
class Policy(nn.Module):
    """
    @brief 动作策略
    @details 可以输出一个最优动作或者一个动作概率分布，默认高斯分布
    """

    def __init__(
        self, imageSize=[1, 160, 160], action_dim=5,
    ):
...
class QNet(nn.Module):
    ...
    def __init__(
        self, imageSize=[1, 128, 128], stateSize=[5,],
    ):
