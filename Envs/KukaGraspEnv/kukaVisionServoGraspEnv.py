...
from pybullet_envs.bullet.kukaGymEnv import kuka
import numpy as np
...
class KukaNonVisionServoGraspEnv(KukaGymEnv):
  ...
  """
    ...
    """
        ...
        self.observation_space = spaces.Box(
            low=0, high=255, shape=(1, self._height, self._width)
        )
        self.viewer = None
    ...
    """
        ...
        for urdf_name in urdfList:
            ...
            ypos = self._blockRandom * (random.random() - 0.5)
    ...
    """
        ...
        reward = self._reward()
        if reward > 0.8:
            self.success_grasp_times += 1

        debug = {"grasp_success": self._graspSuccess}
        if done:
            self.total_grasp_times += 1
            print(
                f"done: {done}, reward: {reward}, success_grasp_times: {self.success_grasp_times}, total_grasp_times: {self.total_grasp_times}"
            )
