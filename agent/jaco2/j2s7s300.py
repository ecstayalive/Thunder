"""
J2s7s300's basic control system
...
class J2s7s300(Jaco2Agent):
    """Robotic arm control architecture of model J2s7s300"""
    ...
    def __init__(
        ...
        self.jaco_finger_motor = [0.7, 0.7, 0.7]  # << 机械臂抓取器的初始值
    ...
    def reset(self):
        ...
        for _ in range(int(self.action_apply_time / 4)):
            p.stepSimulation()
    ...
    def apply_action(self, action):
        ...
        """
        # Restrict the open and close angle of the gripper
        ...
        finger_angle = -0.9 * action[4] + 1.4
        finger_tip_angle = -1.5 * action[4] + 2
        for i in [8, 10, 12]:
            p.setJointMotorControl2(
                self.jaco_uid,
                i,
            ...
            if action[len(action) - 1] == 1 and i >= 40:
                p.setJointMotorControl2(
                    self.jaco_uid,
                    i + 1,
                    p.POSITION_CONTROL,
                    targetPosition=finger_tip_angle,
    ...
    def set_original_position(self):
        ...
        for item in enumerate(self.motor_indices[:6]):
            p.setJointMotorControl2(
                ...
                jointIndex=item[1],
                controlMode=p.POSITION_CONTROL,
                targetPosition=jaco_original_position[item[0]],
                force=self.max_force,
            )
        # finger
        for i in [8, 10, 12]:
