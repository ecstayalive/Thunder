...
class GPModel:
    def __init__(
        ...
        self.max_step = env.max_step  # << 环境设定步数，即在这些步中完成任务
        self.pre_train_step = 2000

        # 训练网络的一些配置参数
        self.total_time_steps = total_time_steps  # << 每一个episode总的训练步数
        self.epochs = epochs
        ...
        if os.path.exists(self.file_path + "model.pkl"):
            ...
            if torch.cuda.is_available():
                ...
                self.target_entropy = torch.log(torch.Tensor((self.action_dim,))).item()
    ...
    def learn(
        ...
        collect_step = 0
        while self.env_step <= self.total_time_steps:
            # 收集数据
